...
const Step = ({ icon: Icon, title, description, stepNumber, isLast }) => (
  <div className="relative flex group">
    {/* Timeline Line */}
    {!isLast && (
      <div className="absolute left-8 top-16 bottom-0 w-0.5 bg-slate-300 group-hover:bg-gradient-to-b group-hover:from-cyan-600 group-hover:to-slate-300 transition-all duration-500"></div>
    )}

    {/* Icon Bubble */}
    <div className="relative flex-shrink-0 z-10 w-16 h-16 rounded-full bg-white border-2 border-slate-200 flex items-center justify-center shadow-sm group-hover:border-cyan-500 group-hover:shadow-lg group-hover:shadow-cyan-500/20 transition-all duration-300">
      <Icon className="w-8 h-8 text-slate-500 group-hover:text-cyan-600 transition-colors duration-300" />
      <div className="absolute -top-2 -right-2 w-6 h-6 bg-slate-100 rounded-full border border-slate-300 flex items-center justify-center text-xs font-bold text-slate-600 group-hover:bg-cyan-500 group-hover:border-cyan-500 group-hover:text-white transition-all duration-300">
        {stepNumber}
      </div>
    </div>
    ...
    <div className="ml-8 pb-16 pt-2">
      <h3 className="text-xl font-bold text-slate-800 mb-3 group-hover:text-cyan-700 transition-colors duration-300">
        {title}
      </h3>
...
const Methodology = () => {
  ...
  return (
    <section className="py-24 bg-[#ecf4ff] relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-0 right-0 w-1/3 h-full from-slate-900 via-slate-900/50 to-transparent pointer-events-none"></div>
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          
          {/* Left Column: Headline & Sticky Content */}
          <div className="lg:sticky lg:top-32">
            <div className="inline-block px-4 py-1 rounded-full bg-purple-100 border border-purple-200 text-purple-700 text-sm font-semibold mb-6">
            ...
            <button className="flex items-center space-x-2 text-cyan-700 font-semibold hover:text-cyan-500 transition-colors group">
